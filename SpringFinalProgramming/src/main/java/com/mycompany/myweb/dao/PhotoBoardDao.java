...
public class PhotoBoardDao {
	...
	public int insert(PhotoBoard photoBoard) {
		String sql = "insert into photoDao(bno, btitle, bcontent, bwriter, bhitcountm, bdate, originalfile, savedfile, mimetype) values(seq_photoBoard_bno.nextval, ?, ?, ?, 0, sysdate, ?, ?, ?)";
	...
	public PhotoBoard selectByBno(int bno) {
		...
		List<PhotoBoard> list = jdbcTemplate.query(sql, new Object[]{bno}, new RowMapper<PhotoBoard>() {//이 쿼리문에서 queryForObject를 사용못하는 이유는-- 값이 없을경우도 있기에 사용할수 없다.
			@Override
	...
	public List<PhotoBoard> selectByPage(int pageNo, int rowsPerPage) {
		...
		sql+= "from ( ";
		sql+=  "select rownum as rn, btitle, bhitcount, savedfile ";
	...
	public int count() {
		String sql="select count(*) from photoBoard";//이 쿼리문에서 queryForObject를 사용하는이유-- 행에 대한 하나의 값이 반드시 올때 queryForObject를 사용한다.(하나의값, 하나의행만)
