...
public class ScoreManagement2 { //팀과제 1번
	...
	public static void main(String[] args) throws Exception {
		...
		String[][] students = new String[100][];//String타입 배열을 생성
		String num =null;
		String title = null;
		String name = null;
		String typing = null;
			...
			if( choice.equals("1") ){ //1.목록//조건문에서 비교할때 .equals를 쓴 이유는 참조변수(객체)를 비교해야하기때문이다.(문자열 비교)
				...
				for(String[] student : students){ //students 항목만큼 for문을 돌린다.
					if(student !=null){//배열 student가 null이 아닐때 student배열의 0번째부터 3번째까지 출력한다.//student라는 배열이 아직 생성이 되어있지 않은 상태.
							...
							//출력할때 students라는 배열안에 번지로 저장되어있는 student배열의 값을 출력한다.
					}
				}
				
			} else if( choice.equals("2") ){ //2.글쓰기(입력)
				System.out.println("제목 : ");
				title = scanner.nextLine();//String타입(문자열)의 값을 키보드로 입력받는다.
				System.out.println("글쓴이 : ");
				name = scanner.nextLine();//String타입(문자열)의 값을 키보드로 입력받는다.
				System.out.println("내용 : ");
				typing = scanner.nextLine();//String타입(문자열)의 값을 키보드로 입력받는다.
				for(int i=0; i<students.length; i++){ //변수 i를 지정하고 i를  0~99  [100]까지 1씩 증가 시키면서 돌린다.
					if(students[i] == null){ //배열 students의 값이 null과 같을때 student값을 넣어준다.
						...
						String[] student = { num, title, name, typing, String.valueOf(count)};// String 타입 배열 student에 위에서 입력받은 title name typing을 넣어준다.
						students[i] = student;
						break;
					}
				}
				...
				title = scanner.nextLine(); // 키보드로 입력받는다.
				System.out.println("수정할 내용 ");
				typing = scanner.nextLine(); //키보드로 입력받는다.
