		...
		while(true){//while 반복문을 돌려서 참일때 그번호에 대한 조건문 실행.
			...
			if( choice.equals("1") ){ //1.목록//조건문에서 비교할때 .equals를 쓴 이유는 참조변수(객체)를 비교해야하기때문이다.(문자열 비교)
				...
				for(int i=0; i<students.length; i++){ //변수 i를 지정하고 i를  0~99  [100]까지 1씩 증가 시키면서 돌린다.
					if(students[i] == null){ 
						//배열 students의 값이 null과 같을때 student값을 넣어준다.
						Student2 student = new Student2 ( i, title, name, typing, count);// String 타입 배열 student에 위에서 입력받은 title name typing을 넣어준다.
						students[i] = student;
						break;
					}
				}
				...
				for(Student2 student : students){ //students 항목만큼 for문을 돌린다.
					...
					count++;//카운트 한다.
					students[numchoice].count = count; //int 타입으로 바꿔주었던 count를 다시 String타입으로 변환해준 후에 students[numchoice][4]안에 다시 넣어준다. 
